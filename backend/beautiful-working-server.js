...
app.get('/health', (req, res) => {
  res.json({ 
    status: 'OK', 
    message: 'Beautiful Working Certificate API is running',
...
app.post('/api/certificates', (req, res) => {
  try {
    const { studentName, courseName, completionDate, issuerName, issuerOrganization, description, grade, duration, certificateType, template = 'modern' } = req.body;
    ...
    const certificate = {
      ...
      issuerOrganization: issuerOrganization || 'Blockchain Institute',
      description,
      grade,
      duration,
      certificateType,
      ...
      instituteName: 'Blockchain Institute'
    };
...
// Verify certificate - FIXED to match frontend expectations
...
app.get('/api/certificates/:certificateId/pdf', (req, res) => {
  try {
    ...
    doc.moveDown();
    doc.fontSize(12).text(`Issued by: ${certificate.issuerName}`, { align: 'center' });
    if (certificate.issuerOrganization) {
      doc.fontSize(12).text(`Organization: ${certificate.issuerOrganization}`, { align: 'center' });
    }
    doc.moveDown();
...
// Download certificate as image - FIXED
app.get('/api/certificates/:certificateId/image', (req, res) => {
  try {
    ...
    ctx.fillText(`Issued by: ${certificate.issuerName}`, 600, 450);
    
    ctx.font = '20px Arial';
    ctx.fillText(`Certificate ID: ${certificate.certificateId}`, 600, 550);
    ctx.fillText(`Blockchain Hash: ${certificate.blockchainTxHash}`, 600, 600);
...
console.log('Starting beautiful working certificate server...');

const server = app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 Beautiful Working Certificate API Started!`);
